...
Image_scale(PyObject *obj, PyObject *args, PyObject *kwargs)
{
    ...
    if (self != NULL && PyArg_ParseTuple(args, "ii|ii", &xDim, &yDim, &zDim, &forceVolume))
    {
        try
        {
            ...
            selfScaleToSize(xmippTransformation::LINEAR, I, xDim, yDim, zDim);
            Py_RETURN_NONE;
        }
        catch (XmippError &xe)
        {
...
Image_applyTransforMatScipion(PyObject *obj, PyObject *args, PyObject *kwargs)
{
    ...
    PyObject *wrap = (xmippTransformation::WRAP ? Py_True : Py_False);
    img = self->image->image;
    bool boolOnly_apply_shifts = false;
    bool boolWrap = xmippTransformation::WRAP;
    ...
    try
    {
        ...
        if (PyList_Check(list))
        {
            ...
            img->setScale(scale);
            img->setFlip(flip);
            img->selfApplyGeometry(xmippTransformation::LINEAR, boolWrap, boolOnly_apply_shifts);//wrap, onlyShifts
            Py_RETURN_NONE;
        }
        else
        {
