...
//		Euler_matrix2angles(eulerMat, alpha, beta, gamma, true);
		...
		for (size_t idx = 0; idx<tiltAngles.size(); idx++)
		{
			...
			std::cout << "tiltProj = " << tiltProj << std::endl;
			if (addCoords<1)
			{
			...
			std::cout << "x_2d " << x_2d << "    y_2d " << ycoor << std::endl;
	...
	if (sigmaNoise>0)
	{
		...
		std::cout << "sigma Noise = " << sigmaNoise << std::endl;
		for (size_t idx = 0; idx<tiltAngles.size(); idx++)
		{
			for (int i=0; i<ydim; i++)
			{
				...
				{
					NZYX_ELEM(tiltseriesOut, idx, 0, i, j) =  NZYX_ELEM(tiltseries, idx, 0, i, j)+randomNumber();
				}
			}
		}
	}
