...
void ProgMonoTomo::produceSideInfo()
{
	...
	for (size_t k=0; k<zdimFT; ++k)
	{
		...
		for (size_t i=0; i<ydimFT; ++i)
		{
			...
			for (size_t j=0; j<xdimFT; ++j)
			{
					...
					{
						idxMap.push_back(n);
						freqTomo.push_back(std::max(u, 1e-38f));
					}
					else
					{
						fourierSignal[n] = 0.0f;
						fourierNoise[n] = 0.0f;
					}
					++n;
			}
	...
	//TODO: Check dimensions of Vriez it may fail with the inverse fourier transform
	...
	// TODO DO NOT PRECOMPUTE
	VEC_ELEM(freq_fourier_z,0) = 1e-38;
	for(size_t k=1; k<zdimFT; ++k)
	{
