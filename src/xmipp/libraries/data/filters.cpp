...
void substractBackgroundRollingBall(MultidimArray<double> &I, int radius)
{
    ...
    scaleToSize(xmippTransformation::LINEAR, bgEnlarged, Irolled, XSIZE(I), YSIZE(I));
                          ...
                          CorrelationAux &aux)
{
    ...
    translate(1, Iaux, I1, vectorR2(-shiftX, -shiftY), xmippTransformation::DONT_WRAP);
    ...
    double testY = shiftY;
    translate(1, Iaux, I1, vectorR2(-testX, -testY), xmippTransformation::DONT_WRAP);
               ...
               const MultidimArray<int> *mask, int maxShift, double shiftStep)
{
    ...
    for (double y=-maxShift; y<=maxShift; y+=shiftStep)
    	for (double x=-maxShift; x<=maxShift; x+=shiftStep)
    	{
    		...
    		YY(shift)=y;
    		XX(shift)=x;
    		translate(xmippTransformation::LINEAR,alignedI2,I2,shift,xmippTransformation::DONT_WRAP,0.0);
                   ...
                   RotationalCorrelationAux &aux3)
{
    ...
    for (int i = 0; i < 3; i++)
    {
			...
			((shiftYSR > SHIFT_THRESHOLD) || (shiftYSR < (-SHIFT_THRESHOLD))))
		{
			...
			applyGeometry(xmippTransformation::LINEAR, aux.IauxSR, I, aux.ASR, xmippTransformation::IS_NOT_INV, wrap);
		}
		...
		if (bestRotRS > ROTATE_THRESHOLD)
		{
			...
			applyGeometry(xmippTransformation::LINEAR, aux.IauxRS, I, aux.ARS, xmippTransformation::IS_NOT_INV, wrap);
		}
        ...
        for (size_t objId : MD.ids())
        {
            ...
            if (considerMirror)
                corr = alignImagesConsideringMirrors(Iavg, I(), M, aux, aux2,
                                                     aux3, xmippTransformation::WRAP);
            else
                corr = alignImages(Iavg, I(), M, xmippTransformation::WRAP, aux, aux2, aux3);
            InewAvg += I();
            if (n == 0)
                Iavg = InewAvg;
            if (lastIteration)
                      ...
                      CorrelationAux &aux, VolumeAlignmentAux &aux2)
{
    ...
    R=Raux*R;
    applyGeometry(xmippTransformation::LINEAR, Ifinal, I, R, xmippTransformation::IS_NOT_INV, xmippTransformation::WRAP);
}
                             ...
                             RotationalCorrelationAux &aux)
{
    ...
    rotate(3, I, auxI, -bestRot / 2, xmippTransformation::WRAP);
                 ...
                 RotationalCorrelationAux &aux2, int Niter, bool limitShift)
{
    ...
    for (int i = 0; i < Niter; i++)
    {
        ...
        Iaux.initZeros();
        applyGeometry(xmippTransformation::LINEAR, Iaux, I, A, xmippTransformation::IS_NOT_INV, xmippTransformation::WRAP);
        FOR_ALL_ELEMENTS_IN_ARRAY2D(mask)
        if (!A2D_ELEM(mask,i,j))
            A2D_ELEM(Iaux,i,j) = 0;
    ...
    applyGeometry(xmippTransformation::BSPLINE3, Iaux, I, A, xmippTransformation::IS_NOT_INV, xmippTransformation::WRAP);
    I = Iaux;
    I += avg;
    delete plans;
    return A;
...
void forceDWTSparsity(MultidimArray<double> &V, double eps)
{
    ...
    selfScaleToSize(xmippTransformation::BSPLINE3,V,sizeF,sizeF,sizeF);
    ...
    IDWT(vol_wavelets,V);
    selfScaleToSize(xmippTransformation::BSPLINE3,V,size0, size0, size0);
}
