                      ...
                      double rot0, MultidimArray<int> *mask, double heightFraction)
{
    ...
    for (int k=0; k<(int)ZSIZE(Vin); ++k)
    {
    	...
    	MAT_ELEM(A,2,3)=-k;
    	applyGeometry(xmippTransformation::LINEAR,Vaux,Vin,A,xmippTransformation::IS_NOT_INV,false,0.0);
    	Vout+=Vaux;
		...
		double rotStep, double zmin, double zmax, double zStep, MultidimArray<int> *mask)
{
	...
	applyGeometry(xmippTransformation::LINEAR,V180,Vin,AX,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
	...
	for (double rot=-180; rot<180; rot+=rotStep)
	{
		...
		{
			MAT_ELEM(AZ,2,3)=z;
			applyGeometry(xmippTransformation::LINEAR,Vout,Vin,AZ,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
	...
	applyGeometry(xmippTransformation::BSPLINE3,V180,Vin,AZ*AX,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
	rotation3DMatrix(bestRot/2,'Z',AZ,true);
	MAT_ELEM(AZ,2,3)=bestZ/2;
	applyGeometry(xmippTransformation::BSPLINE3,Vout,Vin,AZ,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
	Vout+=V180;
	Vout*=0.5;
}
