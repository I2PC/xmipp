...
void ProgAngularContinuousAssign2::preProcess()
{
    ...
    // Construct projector
    if (rank==0)
    	projector = new FourierProjector(V(),pad,Ts/maxResol,xmippTransformation::BSPLINE3);
    else
    	projector = new FourierProjector(pad,Ts/maxResol,xmippTransformation::BSPLINE3);
		...
		double a, double b, const Matrix2D<double> &A, double deltaDefocusU, double deltaDefocusV, double deltaDefocusAngle, int degree)
{
	...
	applyGeometry(degree,prm->Ifilteredp(),prm->Ifiltered(),A,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP,0.);
...
//	[            -sin(2*t)*(sx/2 - sy/2), sy + sx*sin(t)^2 - sy*sin(t)^2 + 1]
	...
	return tranformImage(prm,prm->old_rot+deltaRot, prm->old_tilt+deltaTilt, prm->old_psi+deltaPsi,
			a, b, prm->A, deltaDefocusU, deltaDefocusV, deltaDefocusAngle, xmippTransformation::LINEAR);
}
...
//	geoParams.wrap=DONT_WRAP;
	...
	else
	{
		try
		{
			...
			if (XSIZE(Ip())!=XSIZE(I()))
			{
				scaleToSize(xmippTransformation::BSPLINE3,Ip(),I(),XSIZE(Ip()),YSIZE(Ip()));
				I()=Ip();
			}
...
//			A(1,1)=1+p(5);
			...
			applyGeometry(xmippTransformation::BSPLINE3,Ip(),I(),A,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
			if (optimizeGrayValues)
			{
