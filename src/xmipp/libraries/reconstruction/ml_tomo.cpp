    ...
    for (int refno = 0; refno < nr_ref; ++refno)
    {
        ...
        for (size_t objId : md.ids())
        {
            ...
            if (do_keep_angles || dont_align || dont_rotate)
            {
                ...
                selfTranslate(xmippTransformation::LINEAR, Itmp(), my_offsets, xmippTransformation::DONT_WRAP);
            }
            else
            {
            ...
            selfApplyGeometry(xmippTransformation::LINEAR, Itmp(), my_A, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP, 0.);
...
ProgMLTomo::produceSideInfo2(int nr_vols)
{
    ...
    else
    {
        ...
        for (size_t objId : MDref.ids())
        {
            ...
            selfApplyGeometry(xmippTransformation::LINEAR, img(), E_rot, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP,
                              DIRECT_MULTIDIM_ELEM(img(), 0));
            selfApplyGeometry(xmippTransformation::LINEAR, img(), E_rot, xmippTransformation::IS_INV, xmippTransformation::DONT_WRAP,
                              DIRECT_MULTIDIM_ELEM(img(), 0));
            Iref.push_back(img);
            Iold.push_back(img);
...
ProgMLTomo::postProcessVolume(Image<double> &Vin, double resolution)
{
    ...
    if (mysampling.SL.symsNo() > 0)
    {
        ...
        for (int isym = 0; isym < mysampling.SL.symsNo(); isym++)
        {
            ...
            R(3, 1) = sh(1) * dim;
            R(3, 2) = sh(2) * dim;
            applyGeometry(xmippTransformation::LINEAR, Vaux(), Vin(), R.transpose(), xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP,
...
ProgMLTomo::precalculateA2(std::vector<Image<double> > &Iref)
{
    ...
    for (int refno = 0; refno < nr_ref; refno++)
    {
        ...
        for (int angno = 0; angno < nr_ang; angno++)
        {
            ...
            applyGeometry(xmippTransformation::LINEAR, Maux, Iref_refno, A_rot_inv, xmippTransformation::IS_NOT_INV,
            		xmippTransformation::DONT_WRAP, DIRECT_MULTIDIM_ELEM(Iref_refno,0));
                                   ...
                                   int &opt_refno, int &opt_angno, Matrix1D<double> &opt_offsets)
{
    ...
    if (do_mask)
    {
        ...
        A_rot_inv = A_rot.inv();
        applyGeometry(xmippTransformation::LINEAR, Mmask, Imask(), A_rot_inv, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP, 0.);
        Maux *= Mmask;
    }
        ...
        for (int aa = old_optangno; aa < old_optangno + my_nr_ang; aa++)
        {
            ...
            if (is_a_neighbor)
            {
                ...
                for (int rr = old_optrefno; rr < old_optrefno + nr_ref; rr++)
                {
                    ...
                    applyGeometry(xmippTransformation::LINEAR, Maux2, Iref[refno](), A_rot_inv, xmippTransformation::IS_NOT_INV,
                                  xmippTransformation::DONT_WRAP, DIRECT_MULTIDIM_ELEM(Iref[refno](),0));
                    ...
                    if (my_maxweight > SIGNIFICANT_WEIGHT_LOW * maxweight)
                    {
                        ...
                        selfApplyGeometry(xmippTransformation::LINEAR, Maux, A_rot, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP,
        ...
        Matrix1D<double> &opt_offsets)
{
        ...
        for (int aa = old_optangno; aa < old_optangno + my_nr_ang; aa++)
        {
            ...
            if (is_a_neighbor)
            {
                ...
                for (int rr = old_optrefno; rr < old_optrefno + nr_ref; rr++)
                {
                    ...
                    applyGeometry(xmippTransformation::LINEAR, Maux, Iref[refno](), A_rot_inv, xmippTransformation::IS_NOT_INV,
                                  xmippTransformation::DONT_WRAP, DIRECT_MULTIDIM_ELEM(Iref[refno](),0));
                    local_transformer.FourierTransform();
                    if (do_missing)
                    {
    ...
    YY(opt_offsets) = -(double) iopty;
    ZZ(opt_offsets) = -(double) ioptz;
    selfTranslate(xmippTransformation::LINEAR, Mimg0, opt_offsets, xmippTransformation::DONT_WRAP);
    A_rot = (all_angle_info[opt_angno]).A;
    maskSphericalAverageOutside(Mimg0);
    selfApplyGeometry(xmippTransformation::LINEAR, Mimg0, A_rot, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP,
    ...
    //try
    {
        ...
        while (distributor->getTasks(firstIndex, lastIndex))
        {
            for (size_t imgno = firstIndex; imgno <= lastIndex; ++imgno)
            {
                ...
                {
                    selfTranslate(xmippTransformation::LINEAR, img(), prm->imgs_optoffsets[imgno], xmippTransformation::DONT_WRAP);
                }
