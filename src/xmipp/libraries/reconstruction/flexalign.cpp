...
#include <algorithm>
#include "reconstruction/flexalign.h"
#include "core/xmipp_image_generic.h"

template<typename T>
void FlexAlign<T>::readParams() {
    fnMovie = getParam("-i");
    fnOut = getParam("-o");
...
template<typename T>
void FlexAlign<T>::checkSettings() {
...
template<typename T>
void FlexAlign<T>::loadFrame(const MetaData& movie,
...
template<typename T>
T FlexAlign<T>::getPixelResolution(T scaleFactor) {
    return this->Ts / scaleFactor;
}

template<typename T>
MultidimArray<T> FlexAlign<T>::createLPF(T Ts, const Dimensions &dims) {
    assert(Ts >= this->Ts);
...
template<typename T>
void FlexAlign<T>::createLPF(T Ts, MultidimArray<T> &filter) {
    // from formula
...
template<typename T>
void FlexAlign<T>::scaleLPF(const MultidimArray<T>& lpf,
...
template<typename T>
void FlexAlign<T>::computeTotalShift(int iref, int j,
...
template<typename T>
int FlexAlign<T>::findReferenceImage(size_t N,
...
template<typename T>
void FlexAlign<T>::loadDarkCorrection(Image<T>& dark) {
    if (fnDark.isEmpty())
        return;
    dark.read(fnDark);
    if (yDRcorner != -1)
...
template<typename T>
void FlexAlign<T>::loadGainCorrection(Image<T>& igain) {
    if (fnGain.isEmpty())
        return;
    igain.read(fnGain);
    if (yDRcorner != -1)
...
template<typename T>
void FlexAlign<T>::readMovie(MetaData& movie) {
...
template<typename T>
void FlexAlign<T>::storeGlobalShifts(
...
template<typename T>
AlignmentResult<T> FlexAlign<T>::loadGlobalShifts(MetaData &movie) {
...
template<typename T>
void FlexAlign<T>::setZeroShift(MetaData& movie) {
...
template<typename T>
AlignmentResult<T> FlexAlign<T>::computeAlignment(
...
template<typename T>
void FlexAlign<T>::storeResults(Image<T>& initialMic,
...
template<typename T>
void FlexAlign<T>::correctLoopIndices(
...
template<typename T>
void FlexAlign<T>::printGlobalShift(
...
template<typename T>
void FlexAlign<T>::setNoOfPaches(const Dimensions &movieDim,
...
template class FlexAlign<double> ;
