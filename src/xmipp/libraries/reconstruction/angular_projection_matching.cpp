        ...
        double &maxcorr)
{
    ...
    rotate(xmippTransformation::BSPLINE3,Mref,proj_ref[refno],opt_psi,xmippTransformation::DONT_WRAP);
    ...
    if (opt_flip)
    {
        ...
        MAT_ELEM(A,0, 0) = -1.;
        //MAT_ELEM(A,0, 1) *= -1.;
        applyGeometry(xmippTransformation::LINEAR, Mimg, img, A, xmippTransformation::IS_INV, xmippTransformation::DONT_WRAP);
    }
    else
        Mimg = img;
    ...
    translate(xmippTransformation::LINEAR,Mtrans,Mimg,vectorR2(opt_xoff,opt_yoff),true);
        ...
        double &maxcorr)
{
    ...
    applyGeometry(xmippTransformation::LINEAR, Mref, proj_ref[refno], A, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP);
        ...
        scale += 0.01 * scale_step)
    {
        ...
        A.initIdentity();
        A /= scale;
        applyGeometry(xmippTransformation::LINEAR, Mscale, Mtrans, A, xmippTransformation::IS_INV, xmippTransformation::DONT_WRAP);
...
void ProgAngularProjectionMatching::getCurrentImage(size_t imgid, Image<double> &img)
{
    ...
    if (!A.isIdentity())
        selfApplyGeometry(xmippTransformation::BSPLINE3, img(), A, xmippTransformation::IS_INV, xmippTransformation::WRAP);
    //img.write("after.spi");
    //exit(0);
}
