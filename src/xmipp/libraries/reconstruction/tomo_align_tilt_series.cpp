...
public:
    ...
    double affine_fitness_individual(double *p)
    {
        ...
        for (size_t level=0; level<I1.size(); level++)
        {
            ...
            applyGeometry(xmippTransformation::LINEAR,transformedI1,*(I1[level]),A12level,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
            applyGeometry(xmippTransformation::LINEAR,transformedI2,*(I2[level]),A21level,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);

            // Produce masks for the comparison
            MultidimArray<int> maskInTheSpaceOf1, maskInTheSpaceOf2;
            MultidimArray<double> maskAux;
            applyGeometry(xmippTransformation::LINEAR,maskAux,Mask1_level,A12level,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
            ...
            maskAux.initZeros();
            applyGeometry(xmippTransformation::LINEAR,maskAux,Mask2_level,A21level,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
                        ...
                        bool checkRotation, int pyramidLevel)
{
    ...
    do
    {
        ...
        *dummy=Mask2;
        ...
        {
            selfScaleToSize(xmippTransformation::LINEAR,I1aux,YSIZE(I1aux)/2,XSIZE(I1aux)/2);
            selfScaleToSize(xmippTransformation::LINEAR,I2aux,YSIZE(I2aux)/2,XSIZE(I2aux)/2);
            selfScaleToSize(xmippTransformation::LINEAR,Mask1,YSIZE(Mask1)/2,XSIZE(Mask1)/2);
            selfScaleToSize(xmippTransformation::LINEAR,Mask2,YSIZE(Mask2)/2,XSIZE(Mask2)/2);
        ...
        bool tryFourier) const
{
    ...
    if (tryFourier)
    {
            ...
            YY(rjj)+halfSize<=FINISHINGY(Ijj))
        {
            ...
            selfTranslate(xmippTransformation::LINEAR,piecejj,fftShift,xmippTransformation::WRAP);
...
void ProgTomographAlignment::alignImages(const Alignment &alignment)
{
    ...
    for (int n=0;n<Nimg; n++)
    {
        ...
        M=M1*M2*M3;
        selfApplyGeometry(xmippTransformation::BSPLINE3,I(),M,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
        selfApplyGeometry(xmippTransformation::LINEAR,mask,M,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
        mask.binarize(0.5);
        typeCast(mask,iMask);
        ...
        if (fnSelOrig!="")
        {
            ...
            M=M1*M2*M3;
            selfApplyGeometry(xmippTransformation::BSPLINE3,Iorig(),M,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
            selfApplyGeometry(xmippTransformation::LINEAR,mask,M,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP);
            mask.binarize(0.5);
            typeCast(mask,iMask);
