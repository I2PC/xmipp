...
void ProgML2D::expectationSingleImage(Matrix1D<double> &opt_offsets)
{
    ...
    {
        Maux.setXmippOrigin();
        applyGeometry(xmippTransformation::LINEAR, Maux, Mimg, F[iflip], xmippTransformation::IS_INV, xmippTransformation::WRAP);
    ...
    if (model.do_norm)
    {
        ...
        selfTranslate(xmippTransformation::LINEAR, Maux2, opt_offsets, true);
        selfApplyGeometry(xmippTransformation::LINEAR, Maux2, F[iopt_flip], xmippTransformation::IS_INV, xmippTransformation::WRAP);
...
void ProgML2D::doThreadRotateReferenceRefno()
{
    ...
    FOR_ALL_THREAD_REFNO()
    {
        ...
        for (size_t ipsi = 0; ipsi < nr_psi; ipsi++)
        {
            ...
            rotate(xmippTransformation::BSPLINE3, Maux, model.Iref[refno](), -psi, 'Z', xmippTransformation::WRAP);
...
void ProgML2D::doThreadReverseRotateReferenceRefno()
{
    ...
    FOR_ALL_THREAD_REFNO()
    {
        ...
        for (size_t ipsi = 0; ipsi < nr_psi; ipsi++)
        {
            ...
            rotate(xmippTransformation::BSPLINE3, Maux2, Maux3, psi, 'Z', xmippTransformation::WRAP);
    ...
    FOR_ALL_THREAD_REFNO()
    {
        if (!limit_rot || pdf_directions[refno] > 0.)
        {
            ...
            for (int imirror = 0; imirror < nr_mirror; imirror++)
            {
                ...
                {
                    translate(xmippTransformation::LINEAR, Mtrans, Mimg, trans, true);
                    for (size_t iflip = 0; iflip < nr_nomirror_flips; iflip++)
                    {
                        applyGeometry(xmippTransformation::LINEAR, Mflip, Mtrans, F[IIFLIP], xmippTransformation::IS_INV, xmippTransformation::WRAP);
                        for (size_t ipsi = 0; ipsi < nr_psi; ipsi++)
                        {
                            diff = A2_plus_Xi2;
