...
**
*/
void ProgReconsSuper::Reconstruct2D(MultidimArray<double> &X, MultidimArray<double> PD, std::vector<double> Angles)
{
 ...
 if(Y_file == true){
    ...
    x.resize(X.zdim,X.xdim);
    v.resize(X.zdim,X.xdim);
    z.resize(X.zdim,X.xdim);
   }
 ...
 for(uint plane=0; plane<PD.ydim; plane++){
     ...
     //memset(splane,0,6*sizeof(char));
     //sprintf(splane,"%04d",plane); 
     //scnt = std::string(splane); 
     //oname = "Pr_Phi_" + scnt + ".dat";
     //outfPr.open(oname, std::ios::binary | std::ios::out);
     //outfPr.write(reinterpret_cast<char *>(&k), sizeof(int)); // sizeof can take a type
     ...
     while(true){
         ...
         std::cout << "\e[1m\e[33m" << "Executing Operator B: " << plane << "\e[m\e[0m";
         std::cout << std::endl;
         B(x,PD,Angles,plane,k);
         phi.postupdate(x);
         k += 1;
         
         double lsqv = B.Pr(x,PD,Angles,plane);
         double tphi = phi(x);
         ...
         //outfPr.write(reinterpret_cast<char *>(&lsqv), sizeof(double));
         //outfPr.write(reinterpret_cast<char *>(&tphi), sizeof(double));
     ...
     //outfPr.seekp(0,std::ios_base::beg);
     //outfPr.write(reinterpret_cast<char *>(&k), sizeof(int));
     //outfPr.close();
     //
     // Transfering reconstructed plane to the volume
     //
     ReturnSlice(x, plane, X);
    }
}
...
**
*/
void ProgReconsSuper::ReadTiltSeries(std::vector<double>& Angles, MultidimArray<double>& TS) 
{
 #ifdef XMIPP_21
...
**
*/
void ProgReconsSuper::Create3DImage(const MultidimArray<double>& TS, MultidimArray<double>& x) 
{
 if(Y_file == true){
    std::string pFileName("tmp_file_sup.bin");

    hdrOff = getMRCHdrSize();
    /*
    char* pFileName = NULL;
   ...
   if(!pFileName){
      std::cerr << "Couldn't create temp file name."<<std::endl;
      throw std::iostream::failure("");
      return;
     }
    
    fX.open(pFileName, std::ios::in | std::ios::out | std::ios::app | std::ios::binary);
    ...
    */
    ...
    if(X0_rand == true){
...
#endif
       for(uint slice=0 ; slice<TS.ydim ; slice++){
           for(long j=0; j<(x.xdim*x.ydim*x.zdim); j++){
#ifdef NORMAL_DIST
               x.data[j] = Pmin + 0.5*std::fabs(Pmax-Pmin) +
                           0.25*std::fabs(Pmax-Pmin)*distribution(generator);
#else                          
               x.data[j] = distribution(generator);
#endif
              }
           //fX.write((char*)&x.data[0], x.xdim*x.ydim*x.zdim*sizeof(double));
           long n_data = std::fwrite(x.data, sizeof(double), x.xdim*x.ydim*x.zdim, fX);
           if(n_data != (x.xdim*x.ydim*x.zdim)){
              std::cout << "Unable to write into file:"<<n_data<<std::endl;
              throw std::iostream::failure("");
              return;
             }
          }
       ...
       for(uint slice=0 ; slice<TS.ydim ; slice++){
           ...
           long n_data = std::fwrite(x.data, sizeof(double), x.xdim*x.ydim*x.zdim, fX);
           if(n_data != (x.xdim*x.ydim*x.zdim)){
              std::cout << "Unable to write into file: "<<n_data<<", "<<(x.xdim*x.ydim*x.zdim)<<std::endl;
              throw std::iostream::failure("");
              return;
             }
          }
    ...
    if(X0_file == true){
       Image<double> IX0;
       IX0.read(X0_name); // Reading file
       memcpy(x.data, IX0.data.data, x.xdim*x.ydim*x.zdim*sizeof(double));;
      }
    if(X0_rand == true){
       ...
       std::default_random_engine generator(seed);
#ifdef NORMAL_DIST
       td::normal_distribution<double> distribution(0.0,1.0);  // <--- Normal distribution with 0 mean and std. dev. = 1.0
#else
       std::uniform_real_distribution<double> distribution(Pmin,Pmax);
#endif
...
**
*/
void ProgReconsSuper::Save3DImage(MultidimArray<double>& TS, MultidimArray<double>& x) 
{
 if(Y_file == true){
    uint xdim = TS.xdim,
         ydim = Zsize,
    ...
    TS.clear();
    x.clear();
    x.resize(zdim,ydim,xdim);
    memset(x.data,0,x.xdim*x.ydim*x.zdim*sizeof(double));
    //fX.seekg(0, std::ios::beg);
    //fX.read((char*)&x.data[0], x.xdim*x.ydim*x.zdim*sizeof(double));
    //fX.close();
    std::fseek(fX, 0, SEEK_SET);
    writeMRCHdr(fX,TS.xdim,Zsize,TS.ydim);
    
    /*
    if(std::fread(x.data, sizeof(double), x.xdim*x.ydim*x.zdim, fX) != (x.xdim*x.ydim*x.zdim)){
       std::cout << "Unable to read the file content"<<std::endl;
       throw std::iostream::failure("");
       return;
      }
    */
...
**
*/
void ProgReconsSuper::LoadSlice(const MultidimArray<double>& x, const uint plane, MultidimArray<double>& S)
{
 if(Y_file == true){
    memset(x.data,0,x.xdim*x.ydim*x.zdim*sizeof(double));
    
    //fX.seekg(plane*x.xdim*x.ydim*sizeof(double), std::ios::beg);
    //fX.read((char*)&x.data[0], x.xdim*x.ydim*x.zdim*sizeof(double));
    /*
    std::fseek(fX, plane*x.xdim*x.ydim*sizeof(double), SEEK_SET);
    if(std::fread(x.data, sizeof(double), x.xdim*x.ydim*x.zdim, fX) != (x.xdim*x.ydim*x.zdim)){
       std::cout << "Unable to read the file content"<<std::endl;
       throw std::iostream::failure("");
       return;
    ...
    */
    ...
    for(uint z=0;z<x.zdim;z++)
        memcpy(&S.data[z*S.xdim + 0], &x.data[z*S.xdim*S.ydim + plane*S.xdim + 0], x.xdim*sizeof(double));
   }
}
...
**
*/
void ProgReconsSuper::ReturnSlice(const MultidimArray<double>& S, const uint plane, MultidimArray<double>& x)
{
 if(Y_file == true){
    memcpy(&x.data[0], &S.data[0], S.xdim*S.ydim*sizeof(double));
    try{
       //fX.seekg(plane*S.xdim*S.ydim*sizeof(double), std::ios::beg);
       //fX.write(reinterpret_cast<char*>(S.data), S.xdim*S.ydim*sizeof(double));
       std::fseek(fX, hdrOff + plane*S.xdim*S.ydim*sizeof(double), SEEK_SET);
       if(std::fwrite(S.data, sizeof(double), S.xdim*S.ydim, fX)              != (S.xdim*S.ydim)){
          std::cout << "Unable to write into the file content"<<std::endl;
          throw std::iostream::failure("");
          return;
         }
      }
    ...
    for(uint z=0;z<x.zdim;z++)
        memcpy(&x.data[z*S.xdim*S.ydim + plane*S.xdim + 0], &S.data[z*S.xdim + 0], x.xdim*sizeof(double));
   }
}
...
**
*/
double ProgReconsSuper::Pr(const MultidimArray<double>& X,const MultidimArray<double>& P, const std::vector<double>& Angles)
{
 double lsqv = 0.0;
...
**
*/
double ProgReconsSuper::Phi(const MultidimArray<double>& X, const MultidimArray<double>& P, const std::string type)
{
 double phiv = 0.0;
...
void ProgReconsSuper::defineParams()
{
 ...
 addParamsLine("  [--yfile <y = 0>]       : The reconstructed volume is stored in a temporary file. If 0, then the program tries to load the whole volume in memory, otherwise is stored in a temporary file.");
...
void ProgReconsSuper::show()
{
 if(verbose > 0){
    ...
    if(Y_file == false)
       std::cout << "The program will try to load te whole dataset in memory" << std::endl;
    else
       std::cout << "The program will use a temporary file to store the 3D image" << std::endl;
...
void ProgReconsSuper::checkArgsInfo()
{
 ...
 //
 // Checking the value for the size of the block in the Y direction
 //
 if(Y_file == false){
    std::cerr << "\e[1m\e[1;31m" << "WARNING: " << "\e[m\e[0m";
    std::cerr << "yfile was not set. The program will try to handle all the information using main memory." << std::endl;
   }
...
void ProgReconsSuper::run()
{
 ...
 std::cout << Pr(x,TS,tiltAngles) << std::endl;
 
 //std::cout << "\e[1m\e[34m" << "TV" << "\e[m\e[0m";
 //std::cout << ": " << std::setprecision(12);
 //std::cout << Phi(x,std::string("ITV")) << std::endl;
