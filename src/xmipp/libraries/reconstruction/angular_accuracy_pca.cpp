...
void ProgAngularAccuracyPCA::obtainPCAs(MetaData &SF, size_t numPCAs)
{
    ...
    for (size_t objId : SF.ids())
	{
	    ...
	    selfApplyGeometry(xmippTransformation::BSPLINE3, img(), Trans, xmippTransformation::IS_NOT_INV, xmippTransformation::WRAP);
		...
		typeCast(img(), temp);
		selfScaleToSize(xmippTransformation::LINEAR,temp,newXdim,newYdim,1);
		temp.resize(newXdim*newYdim);
		temp.statisticsAdjust(0.f,1.f);
		pca.addVector(temp);
		imgno++;
    ...
    for (size_t objId : SF.ids())
	{
		...
		double angle = atan2(MAT_ELEM(E,0,1),MAT_ELEM(E,0,0));
		angle = -(angle*180)/3.14159;
		selfRotate(xmippTransformation::LINEAR, P(),angle , xmippTransformation::WRAP);
		typeCast(P(), temp);
		selfScaleToSize(xmippTransformation::LINEAR,temp,newXdim,newYdim,1);
		temp.resize(newXdim*newYdim);
		temp.statisticsAdjust(0.f,1.f);
