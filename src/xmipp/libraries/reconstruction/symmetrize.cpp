                      ...
                      const MultidimArray<double> * mask, int Cn)
{
    ...
    if (!helical && !dihedral && !helicalDihedral)
    {
    	...
    	if (spline==3)
    	{
    		produceSplineCoefficients(xmippTransformation::BSPLINE3, Bcoeffs, V_in);
    		BcoeffsPtr=&Bcoeffs;
    	}
        for (int i = 0; i < SL.symsNo(); i++)
        {
            ...
            */
            applyGeometry(spline, V_aux, V_in, R.transpose(), xmippTransformation::IS_NOT_INV, wrap, avg, BcoeffsPtr);
    ...
    else if (helicalDihedral)
    {
        ...
        rotate(spline,Vrotated,V_out,180.0,'X',xmippTransformation::WRAP);
        V_out+=Vrotated;
        V_out*=0.5;
    }
    else if (dihedral)
