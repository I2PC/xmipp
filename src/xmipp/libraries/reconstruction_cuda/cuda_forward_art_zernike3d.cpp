...
// ForwardArtZernike3D methods

ForwardArtZernike3D::ForwardArtZernike3D()
{
}
...
void ForwardArtZernike3D::setupConstantParameters()
{
    if (program == nullptr)
...
void ForwardArtZernike3D::setupChangingParameters()
{
    if (program == nullptr)
        throw new std::runtime_error("ForwardArtZernike3D not associated with the program!");
...
KernelOutputs ForwardArtZernike3D::getOutputs()
{
    return outputs;
}

void ForwardArtZernike3D::transferImageData(Image<double>& outputImage, PrecisionType* inputData)
{
...
void ForwardArtZernike3D::runKernel()
{
...
void ForwardArtZernike3D::setupImageMetaData(const Image<double>& mda)
{
...
void ForwardArtZernike3D::setupImage(Image<double>& inputImage, PrecisionType** outputImageData)
{
    auto& mda = inputImage();
    transformData(outputImageData, mda.data, mda.xdim * mda.ydim * mda.zdim);
}

void ForwardArtZernike3D::setupImage(const ImageMetaData& inputImage, PrecisionType** outputImageData)
{
