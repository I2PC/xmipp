...
if __name__=="__main__":
  ...
  additonalArgs= [("-i", "--input", {
      ...
      ("--noise_stats", {
          ...
          "required": False,
          "help": "(Optional) Normalization mode 1: The statisitcs of the noise to normalize (mean and standard deviation= the input. Prefered over binaryMask but ignored if "
                  "binaryMask provided. If not --noise_stats nor --binaryMask provided, nomralization params will be automatically estimated, although estimation may fail"}),
      ...
      ("-m", "--binaryMask", {
         ...
         "help": "(Optional) Normalization mode 2: A binaryMask (1 protein, 0 no protein) used to normalize the input. If no normalization mode"
                 "provided, automatic option normalization will be carried out"}),  #TODO: Skip computations outside binaryMask
      ...
      ("--cleaningStrengh", {
        ...
        "default": 0.1,
        "required": False,
        "help": "Max size of connected componemts to remove 0<s<1 or -1 to deactivate. Default: %(default)s"
      }),
