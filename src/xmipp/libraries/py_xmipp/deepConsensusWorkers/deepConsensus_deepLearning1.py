...
class DeepTFSupervised(object):
    ...
    def loadNNet(self, kerasModelFname, keepTraining=True, learningRate=1e-4, l2RegStrength=1e-5):
        self.nNetModel = keras.models.load_model(kerasModelFname,
                                                 custom_objects={"DESIRED_INPUT_SIZE": DESIRED_INPUT_SIZE})
    ...
    """
        ...
        for modelNum in range(self.numberOfModels):
            ...
            if os.path.isfile(currentCheckPointName):
                ...
                self.createNet(dataManagerTrain.shape[0], dataManagerTrain.shape[1], dataManagerTrain.shape[2],
                               effective_data_size,
            ...
            self.nNetModel.fit_generator(dataManagerTrain.getTrainIterator(), steps_per_epoch=CHECK_POINT_AT,
                                         validation_data=dataManagerTrain.getValidationIterator(
                                             batchesPerEpoch=n_batches_per_epoch_val),
    ...
    def predictNet(self, dataManger):
        ...
        for modelNum in range(self.numberOfModels):
            ...
            sys.stdout.flush()
            y_pred_all += self.nNetModel.predict_generator(
                (data for data, label in dataManger.getIteratorPredictBatch()),
    ...
    def evaluateNet(self, dataManger):
        ...
        for modelNum in range(self.numberOfModels):
            ...
            y_pred_all[modelNum, :] = self.nNetModel.predict_generator(((data, label) for data, label in
                                                                        dataManger.getIteratorPredictBatch()),
                                                                       steps=n_batches,
            ...
            print("Model %d test (thr=bestMcc) mcc: %f  pre: %f  rec: %f  acc: %f " % (
                modelNum, bestMcc, precision, recall, acc))
            self.closeSession()
        ...
        print(">>>>>>>>>>\nall models mean stats: mcc : %f prec: %f rec: %f  acc: %f roc_auc: %f" % (
            bestMcc, precision, recall, acc, curr_auc))
...
class DataManager(object):
    ...
    """
        ...
        if negSetDict is None:
            validationFraction = -1
        self.mdListFalse = None
        self.nFalse = 0  # Number of negative particles in dataManager

        self.mdListTrue, self.fnMergedListTrue, self.weightListTrue, self.nTrue, self.shape = self.colectMetadata(
            posSetDict)
        ...
        if validationFraction != 0:
            assert 0 not in self.getNBatchesPerEpoch(), "Error, the number of positive particles for training is to small (%d). Must be >> %d" % (
                self.nTrue, BATCH_SIZE)
        else:
            assert self.getNBatchesPerEpoch()[
                       0] != 0, "Error, the number of particles for testing is to small (%d). Must be >> %d" % (
                self.nTrue, BATCH_SIZE)
        ...
        if negSetDict is not None:
            self.mdListFalse, self.fnMergedListFalse, self.weightListFalse, self.nFalse, shapeFalse = self.colectMetadata(
                negSetDict)
    ...
    def getMetadata(self, dataSetNumber=None):
        ...
        if dataSetNumber is None:
            return [mdTrue for mdTrue in self.mdListTrue], [mdFalse for mdFalse in
                                                            self.mdListFalse] if self.mdListFalse else None
        else:
    ...
    def getPredictDataLabel_Id_dataSetNum(self):
        ...
        if self.mdListFalse is not None:
