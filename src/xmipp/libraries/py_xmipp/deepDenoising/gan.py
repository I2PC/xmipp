...
import numpy as np
from keras.layers import (Input, Conv2D, BatchNormalization, LeakyReLU, Dropout, Flatten, Dense, MaxPooling2D)
from keras.models import Model, Sequential
from keras.optimizers import Adam
from keras.utils import multi_gpu_model

from .DeepLearningGeneric import DeepLearningModel
from .dataGenerator import getDataGenerator, extractNBatches
...
class GAN(DeepLearningModel):
    def __init__(self, boxSize, saveModelFname, gpuList="0", batchSize=BATCH_SIZE, modelDepth=4, generatorLoss="MSE",
                 training_DG_ratio=TRAINING_RATIO, loss_logWeight=LOSS_WEIGHT,
    ...
    def _setShape(self, boxSize):
        ...
        if PATCH_SIZE_FRACTION is not None:
            self.img_shape = (
                int(self.img_rows * PATCH_SIZE_FRACTION), int(self.img_cols * PATCH_SIZE_FRACTION), self.channels)
        else:
    ...
    def buildGenerator(self):
        ...
        if os.path.exists(self.saveModelFname):
            ...
            generator = keras.models.load_model(self.saveModelFname,
                                                custom_objects={self.generatorLoss.__name__: self.generatorLoss})
            print("model loaded")
        else:
            generator = build_UNet(self.img_shape, out_ch=1, start_ch=32, depth=self.modelDepth, inc_rate=2.,
                                   activation='relu',
                                   dropout=0.5, batchnorm=True, residual=True, lastActivation="tanh",
                                   l1l2_reg=self.regularizationStrength)
        return generator
    ...
    def train(self, learningRate, nEpochs, xmdParticles, xmdProjections, xmdEmptyParts=None):
        ...
        discriminator_model.compile(loss="binary_crossentropy", metrics=["acc"],
                                    optimizer=optimizer_discriminator_model)
        ...
        generatorGAN_model.compile(loss=[self.generatorLoss, "binary_crossentropy"],
                                   loss_weights=[10 ** self.loss_logWeight, 1.],
        ...
        if nEpochs < 200:
            print(
                "WARNING: The number of epochs is probably too small to train a gan. If bad results, try to use more epochs")
        ...
        for epoch in range(nEpochs):
            ...
            for batchNum, (X_particles, Y_projections) in enumerate(trainIterator):
                ...
                if remainingRoundsToTrainDiscr <= 0:
                    ...
                    for idx in range(0, superBatchSize - self.batchSize, self.batchSize):
                        ...
                        if bool(random.getrandbits(1)):
                            d_loss_fake, acc_fake = discriminator_model.train_on_batch(generated_projections,
                                                                                       fake_labels)
                ...
                if remainingBatchesToTrainGen <= 0:
                    ...
                    generatorLoss = generatorGAN_model.train_on_batch(X_particles2,
                                                                      [Y_projections2, real_labels_no_random])
            ...
            generatorValLoss = generatorGAN_model.evaluate(particles_val, [projections_val,
                                                                           np.ones(projections_val.shape[0])],
                                                           verbose=0)
            ...
            disc_val_loss, disc_val_acc = discriminator_model.evaluate(X_forDiscriminator, Y_forDiscriminator,
                                                                       verbose=0)
            ...
            if generatorValLoss < bestValidationLoss:
                print("Saving model. validation meanLoss improved from %2.6f to %2.6f" % (
...
def build_discriminator(img_shape, nConvLayers=4):
    assert math.log(img_shape[1], 2) > nConvLayers, "Error, too small images: input %s. Min size %s" % (
        img_shape, 2 ** nConvLayers)
