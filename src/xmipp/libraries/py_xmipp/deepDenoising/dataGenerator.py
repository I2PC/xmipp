...
def getDataGenerator(imgsMdXmd, masksMdXmd, xmdEmptyParts=None, augmentData=True, nEpochs=-1, isTrain=True,
                     valFraction=0.1,
                     batchSize=BATCH_SIZE, doTanhNormalize=True, simulateEmptyParts=True, addMismatch=False,
                     path_size_fraction=None):
    if nEpochs < 1:
        nEpochs = 9999999
    ...
    if path_size_fraction is not None and 0 < path_size_fraction < 1:
        shape = tuple([int(path_size_fraction * elem) for elem in shape[:-1]]) + (1,)

    if xmdEmptyParts is not None:
    ...
    def dataIterator(imgFnames, maskFnames, nBatches=None):
        ...
        for epoch in range(nEpochs):
            ...
            for fnImageImg, fnImageMask in zip(imgFnames, maskFnames):
                ...
                if path_size_fraction is not None:
