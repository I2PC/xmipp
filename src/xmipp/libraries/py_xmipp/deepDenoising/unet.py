    ...
    def train(self, learningRate, nEpochs, xmdParticles, xmdProjections, xmdEmptyParts=None):
        ...
        if os.path.isfile(self.saveModelFname):
            ...
            if N_GPUs > 1:
                ...
                model_1gpu = load_model(self.saveModelFname,
        ...
        trainIterator, stepsPerEpoch = getDataGenerator(xmdParticles, xmdProjections, xmdEmptyParts=xmdEmptyParts,
                                                        isTrain=True, augmentData=True, valFraction=0.1,
                                                        batchSize=self.batchSize,
                                                        simulateEmptyParts=self.addSyntheticEmpty)
        cBacks = []
        cBacks += [keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, verbose=1, mode='auto',
                                                     min_lr=1e-8)]
        cBacks += [keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=1, mode='auto')]
        cBacks += [
            AltModelCheckpoint(self.saveModelFname, model_1gpu, monitor='val_loss', verbose=1, save_best_only=True)]
        ...
        model.fit_generator(trainIterator, epochs=nEpochs, steps_per_epoch=self.epochSize,
...
def build_UNet(img_shape, out_ch=1, start_ch=32, depth=3, inc_rate=2., activation='relu', dropout=0.5, batchnorm=False,
    ...
    def level_block(m, dim, depth, inc, acti, do, bn, mp, up, res):
        if depth > 0:
            ...
            m = keras.layers.MaxPooling2D()(n) if mp else keras.layers.Conv2D(dim, 3, strides=2,
                                                                              padding='same',
                                                                              kernel_regularizer=regularizerFun())(n)
    ...
    x = keras.layers.ZeroPadding2D([(pad_size_left, pad_size_right), (pad_size_left, pad_size_right)])(
        i)  # Padded to 2**N
