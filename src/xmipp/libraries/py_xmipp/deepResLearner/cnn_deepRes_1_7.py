...
def Gen(idx):
    while True:
        ...
        while not ok:
            ok = True
            ok = os.system(
                "xmipp_volume_from_pdb  -i %s -o %s --sampling 0.5 --centerPDB -v 0" % (allPDBs[n], fnHash)) == 0
            if ok:
                ok = os.system(
                    "xmipp_transform_filter -i %s.vol -o %sFiltered.vol --fourier low_pass %f 0.02 --sampling 0.5 -v 0" % (
