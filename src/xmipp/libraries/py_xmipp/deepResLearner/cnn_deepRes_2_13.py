...
def Gen(idx):
    while True:
        ...
        maxRes = round(np.random.uniform(1.9, 13), 1)
        ...
        while not ok:
            ok = True
            ok = os.system(
                "xmipp_volume_from_pdb  -i %s -o %s --sampling 1 --centerPDB -v 0" % (allPDBs[n], fnHash)) == 0
            if ok:
                ok = os.system(
                    "xmipp_transform_filter -i %s.vol -o %sFiltered.vol --fourier low_pass %f 0.02 --sampling 1 -v 0" % (
                        fnHash, fnHash, maxRes)) == 0
            if ok:
                ok = os.system(
                    "xmipp_transform_threshold -i %s.vol -o %sMask.vol --select below 0.2 --substitute binarize -v 0" % (
                        fnHash, fnHash)) == 0
            n = np.random.randint(0, len(allPDBs))
            ...
            for y in range(boxDim2, Ydim - boxDim2, 2):
                for x in range(boxDim2, Xdim - boxDim2, 2):
                    if Vmask[z, y, x] > 0:
                        ...
                        if cont2 < batch_size:
                            box = Vf[z - boxDim2:z + boxDim2 + 1, y - boxDim2:y + boxDim2 + 1,
                                  x - boxDim2:x + boxDim2 + 1]
...
if __name__ == '__main__':
    ...
    # checkpoint
    filepath = "/home/erney/data/test_allboxes_dense_512_2-13A_window13_paral/model_w13.h5"
    ...
    model.save('/home/erney/data/test_allboxes_dense_512_2-13A_window13_paral/model_w13_all.h5')
