...
void ProgAngularSphAlignmentGpu::show() const
{
    ...
    //XmippMetadataProgram::show();// show should be const... but in parent, it is not ;(
    std::cout
...
double ProgAngularSphAlignmentGpu::transformImageSph(
        ...
        double deltaDefocusU, double deltaDefocusV, double deltaDefocusAngle)
{
    ...
    // When deform is async, this can be moved down
...
// Predict =================================================================
void ProgAngularSphAlignmentGpu::processImage(const FileName &fnImg, const FileName &, const MDRow &rowIn, MDRow &rowOut)
{
...
void ProgAngularSphAlignmentGpu::writeImageParameters(const FileName &fnImg) {
    ...
    for (size_t j = 0; j < VEC_XSIZE(clnm); j++) {
...
void ProgAngularSphAlignmentGpu::fillVectorTerms(int l1, int l2, Matrix1D<int> &vL1,
        Matrix1D<int> &vN, Matrix1D<int> &vL2, Matrix1D<int> &vM)
{
    int idx = 0;
...
void ProgAngularSphAlignmentGpu::deformVolumeAsync(
        double &def,
        double rot,
        double tilt,
        double psi)
