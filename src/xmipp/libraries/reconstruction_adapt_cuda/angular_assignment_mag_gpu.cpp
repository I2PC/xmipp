...
#include "angular_assignment_mag_gpu.h"

ProgAngularAssignmentMagGpu::ProgAngularAssignmentMagGpu() {
...
ProgAngularAssignmentMagGpu::~ProgAngularAssignmentMagGpu() {
	// release
	CudaFFT<double>::release(planForward);
	CudaFFT<double>::release(planBackward);
}

void ProgAngularAssignmentMagGpu::defineParams() {
...
// Read arguments ==========================================================
void ProgAngularAssignmentMagGpu::readParams() {
...
// Show ====================================================================
void ProgAngularAssignmentMagGpu::show() {
	if (verbose > 0) {
...
void ProgAngularAssignmentMagGpu::startProcessing() {
 ...
 * */
void ProgAngularAssignmentMagGpu::computingNeighborGraph() {
...
void ProgAngularAssignmentMagGpu::computeLaplacianMatrix(Matrix2D<double> &matL,
...
void ProgAngularAssignmentMagGpu::preProcess() {
	...
	MultidimArray<std::complex<double> > MDaRefF_cpu;
	...
	size_t halfDim = Xdim/2 + 1;
	MDaRefF.initZeros(Ydim, halfDim);

	createFFTPlans();

	gpuErrchk(cudaMalloc(&d_data, memsize));
	gpuErrchk(cudaMalloc(&d_f_data, fourier_memsize));
	...
	FOR_ALL_OBJECTS_IN_METADATA(mdRef){
		...
		// fourier CPU
		// applyFourierImage2(MDaRef, MDaRefF_cpu);

		// ************* fourier GPU **************
		applyFourierImageGpu(MDaRef, MDaRefF);
 ...
 * */
void ProgAngularAssignmentMagGpu::graphFourierFilter(Matrix1D<double> &ccVecIn, Matrix1D<double> &ccVecOut ){
...
void ProgAngularAssignmentMagGpu::processImage(const FileName &fnImg,const FileName &fnImgOut,
...
//	applyFourierImage2(MDaIn, MDaInF);
	// ************* fourier GPU **************
	applyFourierImageGpu(MDaIn, MDaInF);
	std::cout<<formatString("size gpu fourier: %d %d",
			(int)YSIZE(MDaInF), (int)XSIZE(MDaInF))<<std::endl;
...
void ProgAngularAssignmentMagGpu::postProcess() {
...
void ProgAngularAssignmentMagGpu::applyCircularMask(
...
void ProgAngularAssignmentMagGpu::applyFourierImage(MultidimArray<double> &data,
...
void ProgAngularAssignmentMagGpu::setSizes(MultidimArray<double> &data){
   	xdim = XSIZE(data);
    ydim = YSIZE(data);
    zdim = ZSIZE(data);
    image_size = xdim * ydim ;
    fourier_size = xdim * (ydim / 2 + 1);
    memsize = image_size * type_size;
    fourier_memsize = fourier_size * complex_size;
}

void ProgAngularAssignmentMagGpu::createFFTPlans(){
    gpu.set();
    size_t Zdim = 1;
    auto forward_settings = FFTSettingsNew<double>{ Xdim, Ydim, Zdim, 1, 1, false, true };
    planForward = CudaFFT<double>::createPlan(gpu, forward_settings);
    planBackward = CudaFFT<double>::createPlan(gpu, forward_settings.createInverse());
}

/* fourier using gpu */
void ProgAngularAssignmentMagGpu::applyFourierImageGpu(
		MultidimArray<double> &h_data,
		MultidimArray<std::complex<double> > &FourierData) {
	gpuErrchk(cudaMemcpy(d_data, h_data.data, memsize, cudaMemcpyHostToDevice));

	CudaFFT<double>::fft(*planForward, d_data, d_f_data);

	gpuErrchk(cudaMemcpy(FourierData.data, d_f_data, fourier_memsize,
...
void ProgAngularAssignmentMagGpu::applyFourierImage2(MultidimArray<double> &data,
		MultidimArray<std::complex<double> > &FourierData) {
...
void ProgAngularAssignmentMagGpu::applyFourierImage3(MultidimArray<double> &data,
		MultidimArray<std::complex<double> > &FourierData, const size_t &ang) {
	(void)ang;
...
void ProgAngularAssignmentMagGpu::getComplexMagnitude(
		MultidimArray<std::complex<double> > &FourierData,
...
MultidimArray<double> ProgAngularAssignmentMagGpu::imToPolar(
...
double ProgAngularAssignmentMagGpu::interpolate(MultidimArray<double> &cartIm,
...
void ProgAngularAssignmentMagGpu::completeFourierShift(MultidimArray<double> &in,
 ...
 */
void ProgAngularAssignmentMagGpu::ccMatrix(const MultidimArray<std::complex<double>> &F1,
...
void ProgAngularAssignmentMagGpu::maxByColumn(MultidimArray<double> &in,
		MultidimArray<double> &out) {
...
/* gets maximum value for each column*/
void ProgAngularAssignmentMagGpu::meanByColumn(MultidimArray<double> &in,
		MultidimArray<double> &out) {
...
void ProgAngularAssignmentMagGpu::maxByRow(MultidimArray<double> &in,
		MultidimArray<double> &out) {
...
/* gets maximum value for each row */
void ProgAngularAssignmentMagGpu::meanByRow(MultidimArray<double> &in,
		MultidimArray<double> &out) {
...
void ProgAngularAssignmentMagGpu::computeCircular() {
...
void ProgAngularAssignmentMagGpu::circularWindow(MultidimArray<double> &in) {
...
void ProgAngularAssignmentMagGpu::psiCandidates(MultidimArray<double> &in,
 ...
 */
void ProgAngularAssignmentMagGpu::bestCand(/*inputs*/
...
/* apply rotation */
void ProgAngularAssignmentMagGpu::applyRotation(const MultidimArray<double> &MDaRef, double &rot,
		MultidimArray<double> &MDaRefRot) {
	// Transform matrix
...
/* apply rotation */
void ProgAngularAssignmentMagGpu::applyRotation(MultidimArray<double> &MDaRef,
...
/* apply translation */
void ProgAngularAssignmentMagGpu::applyShift(MultidimArray<double> &input,
...
/* apply translation */
void ProgAngularAssignmentMagGpu::applyShift(const MultidimArray<double> &input,
		double &tx, double &ty, MultidimArray<double> &output) {
	// Transform matrix
	Matrix2D<double> A(3, 3);
	A.initIdentity();
...
void ProgAngularAssignmentMagGpu::getShift(MultidimArray<double> &ccVector,
...
void ProgAngularAssignmentMagGpu::applyRotationAndShift(const MultidimArray<double> &MDaRef, double &rot, double &tx,
...
void ProgAngularAssignmentMagGpu::applyShiftAndRotation(const MultidimArray<double> &MDaRef, double &rot, double &tx,
		double &ty, MultidimArray<double> &MDaRefRot) {
	// Transform matrix
	Matrix2D<double> A(3, 3);
	A.initIdentity();
