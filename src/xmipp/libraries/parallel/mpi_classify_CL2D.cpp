    ...
    if (prm->alignImages)
    {
		...
		for (int i = 0; i < 3; i++)
		{
				...
				((shiftYSR > SHIFT_THRESHOLD) || (shiftYSR < (-SHIFT_THRESHOLD))))
			{
				...
				ASR.inv(INV);
				applyGeometry(xmippTransformation::LINEAR, IauxSR, I, INV, xmippTransformation::IS_INV, xmippTransformation::WRAP);
			}
			...
			if (bestRotRS > ROTATE_THRESHOLD)
			{
				...
				ARS.inv(INV);
				applyGeometry(xmippTransformation::LINEAR, IauxRS, I, INV, xmippTransformation::IS_INV, xmippTransformation::WRAP);
			}
...
void ProgClassifyCL2D::readParams()
{
	...
	prm = this; // FIXME HACK because of the global variable. Solve it properly
}
