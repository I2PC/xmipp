                                    ...
                                    const EpsilonClass &class_i, bool FTTRI)
{
    ...
    for (int n=0; n<nmax; n++)
    {
        ...
        if (FTTRI)
            ...
            d=alignImages(seed,candidate(),M,xmippTransformation::WRAP,aux,aux2,aux3);
        ...
        while (bestEpsilonClasses.size()!=nref)
        {
            ...
            for (int n=0; n<nmax; ++n)
            {
                ...
                else
                {
                    ...
                    double d1=alignImages(mSeed1,candidate(),M,xmippTransformation::WRAP,aux,aux2,aux3);
                    double d2=alignImages(mSeed2,candidateCopy,M,xmippTransformation::WRAP,aux,aux2,aux3);
                    if (d1>d2)
                        c1.memberIdx.push_back(trueIdx);
                    else
                        c2.memberIdx.push_back(trueIdx);
        ...
        for (size_t i1=0; i1<nref-1; ++i1)
        {
            ...
            for (size_t i2=i1+1; i2<nref; ++i2, ++count)
            {
                if (((count+1)%node->size)==node->rank)
                {
                    ...
                    A2D_ELEM(corr,i2,i1)=A2D_ELEM(corr,i1,i2)=alignImages(centroid_i1,centroid_i2,M,xmippTransformation::WRAP,aux,aux2,aux3);
                }
            }
            if (node->isMaster())
                progress_bar(i1);
    ...
    for (size_t i=0; i<nref; i++)
        if (((i+1)%node->size)==node->rank)
        {
            ...
            for (int n=0; n<nmax; ++n)
            {
                ...
                {
                    candidateCopy=candidate();
                    bestD=alignImages(own_class,candidateCopy,M,xmippTransformation::WRAP,aux,aux2,aux3);
                }
                ...
                for (int neigh=0; neigh<neighMax; neigh++)
                {
                    ...
                    {
                        candidateCopy=candidate();
                        d=alignImages(neighbour,candidateCopy,M,xmippTransformation::WRAP,aux,aux2,aux3);
                    }
    ...
    for (int i=0; i<imax; i++)
    {
        ...
        else
        {
            ...
            for (size_t n=0; n<nmax; n++)
            {
                ...
                candidate.read(fnCandidate);
                candidate().setXmippOrigin();
                A1D_ELEM(distance,n)=1-alignImages(centroid,candidate(),M,xmippTransformation::WRAP,aux,aux2,aux3);
            }
        }
