...
#endif
        // Symmetrize
        symmetrizeVolume(prmCL3Dprog->SL,Paux,P,xmippTransformation::WRAP);
#ifdef DEBUG
        save()=P;
...
void CL3DClass::fitBasic(MultidimArray<double> &I, CL3DAssignment &result)
{
    ...
    else
    {
    	if (prmCL3Dprog->dontAlign)
    		...
    		applyGeometry(xmippTransformation::LINEAR, Iaux, I, A, xmippTransformation::IS_NOT_INV, xmippTransformation::DONT_WRAP);
...
void CL3D::shareAssignments(bool shareAssignment, bool shareUpdates)
{
    ...
    if (shareUpdates)
    {
        ...
        for (int q = 0; q < Q; q++)
        {
            ...
            int listSize;
            for (size_t rank = 0; rank < prmCL3Dprog->node->size; rank++)
            {
                if (rank == prmCL3Dprog->node->rank)
                {
                      ...
                      std::vector<MultidimArray<double> > &_codes0)
{
    ...
    for (int q = 0; q < prmCL3Dprog->Ncodes0; q++)
    {
    ...
    for (size_t _ : prmCL3Dprog->SF.ids())
    {
        if ((imgCounter+1)%prmCL3Dprog->node->size==prmCL3Dprog->node->rank)
        {
            int q = -1;
            if (!initialCodesGiven)
                q = (localCounter+1) % (prmCL3Dprog->Ncodes0);
            size_t objId = prmCL3Dprog->objId[imgCounter];
            ...
            if (!initialCodesGiven)
            {
                ...
                if (prmCL3Dprog->randomizeStartingOrientation)
                {
					...
					selfApplyGeometry(xmippTransformation::LINEAR,I(),E,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP,0.0);
                }
            ...
            else
            {
                ...
                q = -1;
                for (int qp = 0; qp < prmCL3Dprog->Ncodes0; qp++)
                {
                    Iaux = I();
            ...
            if (imgCounter % 100 == 0 && prmCL3Dprog->node->rank == 0)
...
void CL3D::run(const FileName &fnOut, int level)
{
    ...
    while (goOn)
    {
        ...
        int K = XMIPP_MIN(prmCL3Dprog->Nneighbours+1,Q);
        if (K == 0)
            K = Q;
        for (int q = 0; q < Q; q++)
        ...
        for (size_t _ : prmCL3Dprog->SF.ids())
        {
            if ((imgCounter+1)%prmCL3Dprog->node->size==prmCL3Dprog->node->rank)
            {
                ...
                if (prmCL3Dprog->node->rank == 0 && imgCounter % progressStep == 0)
                    progress_bar(imgCounter);
            }
            imgCounter++;
        }
        ...
        if (Q > 1)
        {
            ...
            do
            {
                ...
                if (largestNode == -1 || smallNode == -1)
                    break;
                if (sizeSmallestNode < prmCL3Dprog->PminSize * Nimgs / Q * 0.01 && sizeSmallestNode<0.25*sizeLargestNode)
                {
                    if (prmCL3Dprog->node->rank == 0 && prmCL3Dprog->verbose)
        ...
        if ((iter > 1 && Nchanges < 0.005 * Nimgs && Q > 1) || iter >= prmCL3Dprog->Niter)
            goOn = false;
        iter++;
    }
                     ...
                     bool iterate) const
{
    ...
    do
    {
        ...
        if (iterate)
        {
			for (int it = 0; it < prmCL3Dprog->Niter; it++)
			{
				if (prmCL3Dprog->node->rank == 0 && prmCL3Dprog->verbose >= 2)
				{
    ...
    if (generateAlignedVolumes)
    {
        ...
        for (size_t objId: MDimages.ids())
    	{
    		if (idx%node->size==node->rank)
    		{
    			...
    			applyGeometry(xmippTransformation::BSPLINE3,Valigned(),V(),A,xmippTransformation::IS_NOT_INV,xmippTransformation::DONT_WRAP,0.);
