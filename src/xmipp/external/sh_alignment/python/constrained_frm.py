...
class FixedAngleConstraint(AngularConstraint):
    ...
    def __init__(self, phi, psi, the, nearby):
        ...
        assert 0 <= phi < 360 and 0 <= psi < 360 and 0 <= the < 180
    ...
    def getConstraintVolume(self, bw):
        if self._cv is None or self._bw != bw:
            ...
            get_constraint_vol(cv, bw, self.psi * pi / 180, self.phi * pi / 180, self.the * pi / 180,
                               self.nearby * pi / 180)
    ...
    def toXML(self):
        ...
        xml_obj = etree.Element('AngularConstraint', Type=str(self.type), Phi=str(self.phi), Psi=str(self.psi),
                                Theta=str(self.the), Nearby=str(self.nearby))
...
class FixedAxisConstraint(AngularConstraint):
    ...
    def toXML(self):
        ...
        xml_obj = etree.Element('AngularConstraint', Type=str(self.type), X=str(self.x), Y=str(self.y), Z=str(self.z),
                                Nearby=str(self.nearby))
...
def frm_constrained_align(vf, wf, vg, wg, b, max_freq, peak_offset=None, mask=None, constraint=None, weights=None,
                          position=None, num_seeds=5,
                          pytom_volume=None):
    ...
    if position is None:  # if position is not given, we have to find it ourself
        ...
        numerator, denominator1, denominator2 = frm_correlate(vf, wf, vg, wg, b, max_freq, weights, True, None, None,
                                                              False)
        ...
        vf2 = shift(vf, -position[0] + vf.sizeX() / 2, -position[1] + vf.sizeY() / 2, -position[2] + vf.sizeZ() / 2,
                    'fourier')
    ...
    for i in range(num_seeds):
        ...
        for j in range(max_iter):
            ...
            vf2 = shift(vf, -lm_pos[0] + vf.sizeX() / 2, -lm_pos[1] + vf.sizeY() / 2, -lm_pos[2] + vf.sizeZ() / 2,
                        'fourier')
