...
def frm_idx2angle(bw, i, j, k):
    ...
    phi = pi - ohm
    phi -= 2.0 * pi * floor(phi / 2.0 / pi)  # make it inside [0, 2*PI]
    the = pi - phc
    psi = -phe
    psi -= 2.0 * pi * floor(psi / 2.0 / pi)
...
def frm_angle2idx(bw, phi, psi, the):
    ...
    assert -1 < i < 2 * bw and -1 < j < 2 * bw and -1 < k < 2 * bw
...
def find_subpixel_peak_position(corr, pos):
    ...
    try:
        ...
        peak = corr[x][y][
                   z] + dx * detx + dy * dety + dz * detz + dxx * detx ** 2 / 2 + dyy * dety ** 2 / 2 + dzz * detz ** 2 / 2 + detx * dety * dxy + detx * detz * dxz + dety * detz * dyz
    except:
...
def frm_correlate(vf, wf, vg, wg, b, max_freq, weights=None, ps=False, denominator1=None, denominator2=None,
                  return_score=True):
...
def frm_correlate_cache(vf, wf, vg, wg, b, max_freq, weights=None, ps=False, denominator1=None, denominator2=None,
                        return_score=True):
    """Calculate the correlation of two volumes as a function of Euler angle.
...
def frm_align(vf, wf, vg, wg, b, max_freq, peak_offset=None, mask=None, weights=None, position=None, num_seeds=5):
    ...
    if position is None:  # if position is not given, we have to find it ourself
        ...
        vf2 = translate3d_f(vf, -position[0] + vf.shape[0] / 2, -position[1] + vf.shape[1] / 2,
                            -position[2] + vf.shape[2] / 2)
    ...
    for i in range(num_seeds):
        ...
        for j in range(max_iter):
            ...
            score = frm_correlate_cache(svf2, swf, svg, swg, b, max_freq, weights, False, denominator1, denominator2,
                                        True)
