            ...
            for line in f:
                if line.startswith("#"):
                    ...
                    if len(lineArray) < 1:
                        ...
                        raise ValueError(
                            "Error, input file bad format. If -c option, it must have 2 cols: 'micFname coordsFname'")
                    else:
                        mic_fname = lineArray[0]
                        mic_basename_split = os.path.basename(mic_fname).split(".")
                        mic_basename = ".".join(mic_basename_split[:-1]) if len(mic_basename_split) > 1 else \
                            mic_basename_split[0]
