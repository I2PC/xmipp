...
class ScriptPreproMics(XmippScript):
    ...
    def defineParams(self):
        ...
        self.addParamsLine(
            '-i <inputFile>         : A file that contains the path of input micrograph and possibly CTFs')
        ...
        self.addParamsLine(
            '-o <pathToProcessesMics>  : A path to the directory where preprocessed micrograph will be saved')
            ...
            for line in f:
                if line.startswith("#"):
                    ...
                    if phase_flip:
                        if len(lineArray) != 2:
                            raise ValueError(
                                "Error, input file bad format. If -c option, it must have 2 cols: 'micFname ctfFname'")
                        else:
...
def preproOneMic(mic_fname, samplingRate, out_name, ctf_fname=None, invert_contrast=False, phase_flip=False,
                 downFactor=1):
  ...
  """
    ...
    if downFactor != 1:
        cmd = "xmipp_transform_downsample -i %s -o %s --step %f --method fourier" % (
            mic_fname, out_name_tmp, downFactor)
        print(cmd)
        check_call(cmd, shell=True)
        mic_fname = out_name_tmp

    if phase_flip:
        cmd = "xmipp_ctf_phase_flip -i %s -o %s --ctf %s --sampling %f" % (
            mic_fname, out_name_tmp, ctf_fname, samplingRate * downFactor)
        print(cmd)
        check_call(cmd, shell=True)
        mic_fname = out_name_tmp
