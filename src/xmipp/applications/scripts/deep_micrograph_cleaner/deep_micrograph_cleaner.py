...
class ScriptMicrographCleanerEm(XmippScript):
    ...
    def defineParams(self):
        ...
        self.addParamsLine(
            ' [ -c <inputCoordsDir>  ] : input coordinates directory (.pos or tab separated x y). Filenames ' +
        ...
        self.addParamsLine(
            ' [ -d <deepLearningModel> ]  : (optional) deep learning model filename. If not provided, default model will be used')
        self.addParamsLine('-b <boxSize>     : particles box size in pixels')
        self.addParamsLine(
            '-s <downFactor>   <F=1.0>   : (optional) micrograph downsampling factor to scale coordinates, Default no scaling')
        self.addParamsLine(
            ' [ --deepThr <deepThr> ]: (optional) deep learning threshold to rule out a coordinate. The smaller the treshold ' +
            'the more coordiantes will be rule out. Ranges 0..1. Recommended 0.8')
        self.addParamsLine(
            ' [--sizeThr <sizeThr> <F=0.8> ]: Failure threshold. Fraction of the micrograph predicted as contamination to ignore predictions. ' +
            '. Ranges 0..1. Default 0.8')
        self.addParamsLine(
            '[ --predictedMaskDir <predictedMaskDir> ] : directory to store the predicted masks. If a given mask already existed, it will be used instead' +
            ' of a new prediction')
        ...
        # examples
        self.addExampleLine(
            'xmipp_deep_micrograph_cleaner -c path/to/inputCoords/ -o path/to/outputCoords -b $BOX_SIXE  -i  /path/to/micrographs/')
