...
class ScriptDeepScreeningTrain(XmippScript):
    ...
    def defineParams(self):
        ...
        self.addParamsLine(
            '[ -o <trueTrainData>   ]      : A text file where metada particles (xmd) predictions will be saved. '
            'Requires -n already trained')

        self.addParamsLine(
            '[ --testingTrue <trueTestData>   ]  : A path to metada true particles (curated) (xmd) to be used for '
            'evaluation purposes')
        self.addParamsLine(
            '[ --testingFalse <falseTestData>   ]  : A path to metada false particles (curated) (xmd) to be used for '
            'evaluation purposes')
        ...
        self.addExampleLine(
            'predict particles:  xmipp_deep_screen -n ./netData --score_mode -i unknownParticles.xmd -o '
    ...
    def run(self):
        ...
        if mode in trainKeyWords:
            ...
            if self.checkParam('--trueW'):
                ...
                assert len(trueWeights) == len(
                    trueDataPaths), "Error, the number of weights provided --falseW does not match the " + \
                                    "number of negative data paths provided"
            if self.checkParam('--falseW'):
                falseWeights = self.getParam('--falseW').split(":")
                assert len(falseWeights) == len(
                    falseDataPaths), "Error, the number of weights provided --falseW does not match the " + \
                                     "number of negative data paths provided"
