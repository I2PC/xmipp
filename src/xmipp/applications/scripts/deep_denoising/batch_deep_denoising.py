...
class ScriptDeepDenoising(XmippScript):
    ...
    def defineParams(self):
        ...
        self.addParamsLine(
            ' [ -p <noisyParticles> ] : Projections .xmd to train network (mandatory) or to evaluate denoising')
        self.addParamsLine(
            ' [ --empty_particles <emptyParticles> ] : Empty particles .xmd to make training more robust, optional')
        self.addParamsLine(
            ' [ -o <denoisedParticles> ] : Denoised particles .xmd. Mandatory when mode=="denoising", ignored otherwise')
    ...
    def predictDenoised(self, model, boxSize, inputParticlesMdName, dataPathProjections=None):
        ...
        for preds, particles, projections in model.yieldPredictions(inputParticlesMdName, metadataProjections):
            newRow = md.Row()
            for pred, particle, projection in zip(preds, particles,
        ...
        mdNewParticles.write('particles@' + outputParticlesMdName,
