...
class ScriptAngularDensityAlign(XmippScript):
    ...
    def alignStructure(self, inputStructure, referenceStructure, alignedStructure, contactRadius_list, size):
        ...
        for contactRadius in contactRadius_list:
            ...
            # Compute SPFH
            spfh_input, filtered_input, vectors_input, inputIndeces, inputDelete = self.SPFH(filtered_input, vectors_input, bins, inputIndeces)
            spfh_ref, filtered_ref, vectors_ref, refIndeces, refDelete = self.SPFH(filtered_ref, vectors_ref, bins, refIndeces)
