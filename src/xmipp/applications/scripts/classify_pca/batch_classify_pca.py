    ...
    for i in range(num_batches):
        ...
        if mode:
            ...
            for iter in range(niter):
                ...
                if mode == "create_classes":
                    # if iter < 5:
                    cl, tMatrix, batch_projExp_cpu = bnb.create_classes_version0(mmap, tMatrix, iter, subset, expBatchSize, matches, vectorshift, classes, freqBn, coef, cvecs, mask, sigma)#, maxRes, sampling)
                    # else:
