...
class TomogramReconstruction(XmippScript):
    ...
    def tigreReconstruction(self, ts, tiltAngles):
        ...
        # KRYLOV ALGORITHMS     
            ...
            reconstruction = algs.pscd(ts, geo, tiltAngles, self.iterations, noneg=False, gpuids=gpuids)
            
        elif self.method == 'awpcsd':
            reconstruction = algs.awpscd(ts, geo, tiltAngles, self.iterations, noneg=False, gpuids=gpuids)
