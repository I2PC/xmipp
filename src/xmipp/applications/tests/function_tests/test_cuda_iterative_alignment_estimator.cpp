...
public:
    ...
    void test(const Dimensions &dims, size_t batch) {
        ...
        bool saveOutput = (dims.x() == 260 && dims.y() == 260 && batch == 1);
        ...
        auto maxSize = Dimensions(768, 768, 1, 1000);
        assert(dims.size() < maxSize.size());
        assert(dims.sizeSingle() < maxSize.sizeSingle());

        static auto others = new T[maxSize.size()]();
        T *othersCorrected = nullptr;
        if (saveOutput) {
            othersCorrected = new T[maxSize.size()]();
        }
        ...
        delete[] othersCorrected; // FIXME DS release data in proper function
    }
