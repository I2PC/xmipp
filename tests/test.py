...
def download(destination=None, url=None, verbose=False):
    ...
    for line in lines:
        ...
        try:
            ...
            partial = int(done * 10)
            if verbose:
                print("%3d%% " % (100 * done)), fname
            elif int(done * 100 % 10) == 0 and partial != oldPartial:
...
def update(destination=None, url=None, verbose=False):
...
if __name__ == "__main__":
    ...
    datasetPath = os.environ.get("XMIPP_TEST_DATA", None)
    if os.path.isdir(datasetPath):
        ...
        task = download
    task(destination=datasetPath, url=url)
